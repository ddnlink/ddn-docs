...
- [DDN AOB API文档](#ddn-aob-api文档)
    - [**1 API使用说明**](#1-api使用说明)
        - [**1.1 请求过程说明**](#11-请求过程说明)
    - [**2 接口**](#2-接口)
        - [**2.1 AOB相关交易**](#21-aob相关交易)
            - [**2.1.1 注册资产发行商**](#211-注册资产发行商)
            - [**2.1.2 注册资产**](#212-注册资产)
            - [**2.1.3 资产设置acl模式**](#213-资产设置acl模式)
            - [**2.1.4 更新访问控制列表（acl）**](#214-更新访问控制列表acl)
            - [**2.1.5 资产发行**](#215-资产发行)
            - [**2.1.6 资产转账**](#216-资产转账)
            - [**2.1.7 资产注销**](#217-资产注销)
            - [**2.1.8 其它接口**](#218-其它接口)
        - [**2.2 用户自定义资产aob**](#22-用户自定义资产aob)
            - [**2.2.1 创建资产**](#221-创建资产)
            - [**2.2.2 获取全网所有发行商**](#222-获取全网所有发行商)
            - [**2.2.3 查询指定发行商的信息**](#223-查询指定发行商的信息)
            - [**2.2.4 查看指定发行商的资产**](#224-查看指定发行商的资产)
            - [**2.2.5 获取全网所有资产信息**](#225-获取全网所有资产信息)
            - [**2.2.6 获取指定资产信息**](#226-获取指定资产信息)
            - [**2.2.7 获取指定资产的访问控制列表（acl）**](#227-获取指定资产的访问控制列表acl)
            - [**2.2.8 获取指定账户所有aob的余额**](#228-获取指定账户所有aob的余额)
            - [**2.2.9 获取指定账户所有资产相关操作记录**](#229-获取指定账户所有资产相关操作记录)
            - [**2.2.10 获取指定账户指定资产的余额**](#2210-获取指定账户指定资产的余额)
            - [**2.2.11 获取指定账户指定资产转账记录**](#2211-获取指定账户指定资产转账记录)
            - [**2.2.12 获取指定资产转账记录**](#2212-获取指定资产转账记录)
